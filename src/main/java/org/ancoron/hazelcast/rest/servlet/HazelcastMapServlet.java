...
public class HazelcastMapServlet extends HttpServlet
    ...
    protected void doPut(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (matcher.matches()) {
            ...
            try (final InputStream in = req.getInputStream()) {
                ...
                log("Unable to set value for bucket '" + map + "' and key '" + key + "'", x);
            ...
            throws ServletException, IOException
    {
        ...
        if (matcher.matches()) {
            ...
            final String map = matcher.group(1);
            createBucket(map, 60, 0, 128);
        } else {
            resp.sendError(SC_NOT_FOUND);
        }
    }

    public void createBucket(String map, int ttl, int redundancy, int mib)
            throws IOException
    {
        ...
        config.setBackupCount(redundancy);
    ...
    public void deleteValue(String map, String key) throws IOException
    {
        ...
        if (getValue(map, key) != null) {
            ...
            throw new FileNotFoundException("Key not found in bucket '" + map + "': " + key);
        }
    }
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (matcher.matches()) {
            ...
            try {
                ...
                resp.sendError(SC_NOT_FOUND, "Bucket not found: " + map);
            }
        } else if ((matcher = ROUTE_BUCKET.matcher(path)).matches()) {
            String map = matcher.group(1);
            resp.setStatus(SC_NO_CONTENT);
